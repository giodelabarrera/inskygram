...
class WallPage extends Component {
  ...
  componentDidMount() {
    ...
    logic
      .retrieveUser(loggedInUsername, undefined, token)
  ...
  onToggleLikeClick = async postId => {
    ...
    try {
      ...
      const postUpdated = await logic.retrievePost(
        postId,
        loggedInUsername,
        token
      );
  ...
  isLiked = likes =>
    likes.find(like => like.user.username === this.props.loggedInUsername)
      ? true
      : false;

  isSaved = (post, savedPosts) =>
    savedPosts.find(savedPost => savedPost.post === post._id) ? true : false;
  ...
  handleLoadMore = page => {
    if (this.state.loadMore) {
      ...
      logic
        .listUserWall(token, loggedInUsername, page)
        .then(newPosts => {
      ...
      <div>
        ...
        <div className="main-wrapper">
          <main className="is-eight-quarters grid-gap-30">
            <section>
              {this.state.posts.length > 0 ? (
                ...
                <div>
                  ...
                  . Upload a photo or follow friends to be able to do what they
                  do
            ...
            <section>
              {this.state.user && (
                <div className="WallPage-currentUser">
                  ...
                  <div className="WallPage-currentUserUser">
                    <h5 className="WallPage-currentUserUsername">
                      <Link
                        to={`/${this.state.user.username}`}
                        className="WallPage-currentUserUsernameLink"
                      >
                        {this.state.user.username}
                      </Link>
                    </h5>
                    <h5 className="WallPage-currentUserName">
